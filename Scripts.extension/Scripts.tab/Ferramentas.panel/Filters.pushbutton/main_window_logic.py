# -*- coding: utf-8 -*-
"""Central WPF workflow for Filter Manager."""
import clr, os, json
...
def _create_filters_safely(doc, pairs, prefix, cat_ids, param_id):
    ...
    for val, name in pairs:
        ...
        if not _filter_is_acceptable(doc, cat_ids, elem_filter):
            ...
            continue
        try:
            with revit.Transaction("Filters - Criar filtro"):
...
def _create_filter_exact(doc, filter_name, value, cat_ids, param_id):
    ...
    try:
        with revit.Transaction("Filters - Criar filtro do esquema"):
...
class MainWindow(forms.WPFWindow):
    ...
    def criar_filtros(self, s, a):
        ...
        failed = 0
        if view is not None:
            with revit.Transaction("Filters - Aplicar filtros"):
    ...
    def recolorir_filtros(self, s, a):
        ...
        failed = 0
        with revit.Transaction("Filters - Recolorir"):
    ...
    def e_rename_filter(self, s, a):
        ...
        try:
            with revit.Transaction("Filters - Renomear filtro"):
    ...
    def e_delete_filter(self, s, a):
        ...
        deleted = 0
        failed = 0
        with revit.Transaction("Filters - Apagar filtros"):
    ...
    def e_aplicar(self, s, a):
        ...
        skipped = 0
        failed = 0
        with revit.Transaction("Filters - Editar filtros"):
            for entry in self._edit_entries:
                try:
    ...
    def _transfer_export(self):
        ...
        if proj_dir and os.path.isdir(proj_dir):
            dlg.InitialDirectory = proj_dir
        dlg.FileName = u"{}_filters.json".format(self._project_stem())
        if not dlg.ShowDialog():
            return
        try:
    ...
    def _transfer_import(self):
        ...
        if bool(self.x_chkImpVista.IsChecked) and created_filters:
            view = self.doc.ActiveView
            with revit.Transaction("Filters - Aplicar filtros importados"):
    ...
    def l_apagar(self, s, a):
        ...
        failed = 0
        processed = []
        with revit.Transaction("Filters - Limpar filtros"):
            for name in sel:
